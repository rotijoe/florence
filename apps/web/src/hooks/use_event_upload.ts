...
import { createEventUploadIntentAction, confirmEventUploadAction } from '@/app/tracks/[trackSlug]/[eventId]/actions'
...
type UseEventUploadProps = {
  eventId: string
  trackSlug: string
  onComplete?: (event: EventResponse) => void
}

type UseEventUploadReturn = {
  status: UploadStatus
  error: string | null
  isUploading: boolean
...
export function useEventUpload({ eventId, trackSlug, onComplete }: UseEventUploadProps): UseEventUploadReturn {
  ...
  const upload = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (intentResult.error || !intentResult.uploadUrl || !intentResult.fileUrl || !intentResult.key) {
