...
describe('TrackEventList', () => {
  ...
  it('renders all events', () => {
    render(<TrackEventList events={mockEvents} trackSlug={trackSlug} userId={userId} />)
  ...
  it('renders empty state when no events', () => {
    render(<TrackEventList events={[]} trackSlug={trackSlug} userId={userId} />)
  ...
  it('makes events clickable with correct links', () => {
    ...
    expect(event1Link).toHaveAttribute('href', '/user-1/tracks/test-track/1')

    const event2Link = screen.getByText('Event 2').closest('a')
    expect(event2Link).toHaveAttribute('href', '/user-1/tracks/test-track/2')
  })
  ...
  it('groups events by date and only shows the date once per group', () => {
    ...
    render(<TrackEventList events={groupedEvents} trackSlug={trackSlug} userId={userId} />)
